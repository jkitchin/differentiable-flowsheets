...
class DistillationColumnParams(ParamsMixin):
    ...
    Attributes:
        ...
        feed_stage: Index of the stage the feed enters, numbered from the
            bottom: 0 is the reboiler and ``n_stages - 1`` is the top stage.
            The feed stage is counted with the stripping section on its liquid
            side (``L[feed_stage] = L_rect + q F``) and with the rectifying
            section on its vapour side (``V[feed_stage] = V_rect``); see
            ``_cmo_section_flows``. Stages above it are rectifying, stages
            below it are stripping.
...
class DistillationColumn:
    ...
    Stage numbering: j = 0 is the reboiler (bottom), j = n_stages - 1 is the
    top stage, with a total condenser above it. The feed enters stage
    ``params.feed_stage``; ``_cmo_section_flows`` defines which section each
    stage's flows belong to.
