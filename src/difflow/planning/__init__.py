...
reduced Jacobian **is** the delta vector, and it costs ``min(n_u, n_y)`` AD
passes.  The gradient of a scalar objective costs a small constant multiple of
one model evaluation whatever ``n`` is, while central differences cost exactly ``2n`` — so the advantage
