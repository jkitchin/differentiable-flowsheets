...
Every probe is at pH 1.5 or below, inside D2EHPA's post-#270 validity window
of [0.0, 2.0]; the module used to sit at pH 2.4-2.5. Where a synthetic ``a``
was chosen to put D somewhere interesting, it moved with the pH: b = 3, so a
pH shift of 1.0 is a shift of 3.0 in ``a`` for the same D.
"""
...
class TestOverrideArithmetic:
    def test_a_decade_in_a_is_a_decade_in_D(self, base):
        ...
        assert float(bumped.get_D("Nd", pH=1.5)) == pytest.approx(
            10.0 * float(base.get_D("Nd", pH=1.5)), rel=1e-12)
    ...
    def test_an_untouched_element_is_untouched(self, base):
        ...
        assert float(d.get_D("Dy", pH=1.5)) == pytest.approx(
            float(base.get_D("Dy", pH=1.5)), rel=1e-12)
    ...
    def test_the_separation_factor_moves_by_the_coefficient_difference(self, base):
        ...
        b0 = float(base.get_separation_factor("Dy", "Nd", pH=1.5))
        b1 = float(d.get_separation_factor("Dy", "Nd", pH=1.5))
    ...
    def test_overriding_every_coefficient_at_once(self, base):
        d = REEDistribution(
            ...
            coefficient_overrides={"Nd": {"a": -4.0, "b": 2.0, "c": 0.0,
                                          "d": 0.0}})
        # log10(D) = -4 + 2*1.5 + 0, plus the [HA] term, which is zero at the
        # reference concentration this instance is at.
        assert float(d.get_D("Nd", pH=1.5)) == pytest.approx(10.0 ** -1.0,
                                                             rel=1e-12)
...
class TestOverrideValidation:
    ...
    def test_no_override_is_the_default_and_changes_nothing(self, base):
        ...
        assert float(d.get_D("Nd", pH=1.5)) == pytest.approx(
            float(base.get_D("Nd", pH=1.5)), rel=1e-12)
...
class TestTracing:
    ...
    def test_it_jits(self):
        assert float(jax.jit(self._D)(-4.0)) == pytest.approx(
            float(self._D(-4.0)), rel=1e-12)

    def test_it_vmaps_over_a_sample(self):
        a = jnp.linspace(-4.4, -3.6, 16)
...
class TestUnitsPassItThrough:
    ...
    def test_the_unit_sees_the_overridden_D(self, build, base):
        ...
        assert float(unit._distribution.get_D("Nd", pH=1.5)) == pytest.approx(
            10.0 * float(base.get_D("Nd", pH=1.5)), rel=1e-12)
...
class TestThroughACascade:
    ...
    def _raffinate_Nd(a_Nd):
        params = REEExtractorParams(
            n_stages=5.0, extractant="D2EHPA", elements=ELEMENTS, pH=1.5,
    ...
    def test_more_extractable_Nd_leaves_less_in_the_raffinate(self):
        assert float(self._raffinate_Nd(-4.3)) < float(self._raffinate_Nd(-5.0))

    def test_the_cascade_gradient_is_finite_and_signed(self):
        g = float(jax.grad(self._raffinate_Nd)(-5.0))
        assert np.isfinite(g) and g < 0.0

    def test_the_cascade_vmaps_over_a_sample(self):
        out = jax.vmap(self._raffinate_Nd)(jnp.linspace(-5.3, -4.5, 24))
...
class TestTracedStageCount:
    ...
    def test_a_traced_stage_count_is_allowed(self):
        """(#270) The operating point had to move twice over, and both moves
        are about having a gradient to measure rather than about the window.

        It used to watch Dy at pH 2.4 with 1.12 mol/s of REE against 3 mol/s
        of D2EHPA. After the refit that point is doubly dead: D(Dy) = 1 at
        pH -0.37, so Dy is quantitatively extracted anywhere in [0.0, 2.0] and
        the stage count cannot change it; and 1.12 mol/s of REE is past the
        0.50 mol/s the extractant can hold, so the capacity clamp -- not the
        cascade -- sets the split, and it hands Dy a NEGATIVE stage
        derivative, Nd winning the shared extractant away from it as stages
        are added. The measured gradient was -7.5e-40: dead and wrong-signed.

        Watching Nd at pH 0.2 with the feed scaled to 0.34 mol/s (inside the
        0.50 mol/s capacity) puts the unit where a stage count means
        something: 75% extracted, and +4.5e-3 mol/s per stage.
        """
        def raff(n):
            params = REEExtractorParams(
                n_stages=n, extractant="D2EHPA", elements=ELEMENTS, pH=0.2,
                extractant_conc=1.0)
            feed = make_stream({"H2O": 55.0, "Nd": 0.3, "Dy": 0.036},
                               T=298.15, P=101325.0)
            solvent = make_stream({"D2EHPA": 3.0, "kerosene": 9.0},
                                  T=298.15, P=101325.0)
            _, ext, _ = REEExtractor(params)(feed, solvent)
            return get_flows(ext)["Nd"]
